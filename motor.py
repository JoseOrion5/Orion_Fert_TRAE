...
def build_line_for_target(volume_l: float, insumo: Insumo, target_nutrient: str, target_percent: float) -> Optional[FormulaLine]:
    ...
    if teor is None: return None
    massa = compute_mass_kg_for_target(volume_l, target_percent, teor)
    if massa is None: return None
    contrib = {n: compute_contrib_percent(volume_l, massa, t) for n, t in insumo.teor_por_nutriente_pct.items()}
...
def recommend_process_and_aditivos(
    ...
    *,
    ...
    if has_ca or has_micros: add_if_found(selected_quelante)
...
def build_top12_forms(volume_l: float, targets: Dict[str, float], insumos: Sequence[Insumo], use_optimization: bool = False) -> List[List[FormulaLine]]:
    ...
    for idx in range(12):
        ...
        def is_compatible(ins: Insumo) -> bool:
            nonlocal has_ca, has_p_s
            nm = _norm_text(ins.nome)
            teores = ins.teor_por_nutriente_pct
            c = "Ca" in teores or "calcio" in nm or "cálcio" in nm
            ps = "P2O5" in teores or "S" in teores or "fosfato" in nm or "sulfato" in nm
            if has_ca and ps: return False
            if has_p_s and c: return False
            return True
        ...
        for n, _ in NUTRIENT_COLUMNS:
            ...
            if l:
                ...
                remaining = _apply_contrib_to_remaining(remaining, l.contrib_pct)
                update_comp(pick)
        forms.append(merge_lines(lines))
...
def _build_optimized_forms(volume_l: float, targets: Dict[str, float], insumos: Sequence[Insumo]) -> List[List[FormulaLine]]:
    ...
    A_eq = []
    for k in nutrient_keys:
        row = [i.teor_por_nutriente_pct.get(k, 0.0) / 100.0 for i in valid_insumos]
        A_eq.append(row)
            ...
            if res.success:
                ...
                for idx_val, x_val in enumerate(res.x):
                    if x_val > 0.001:
                        ...
                        contrib = {n: compute_contrib_percent(volume_l, x_val, t) for n, t in ins.teor_por_nutriente_pct.items()}
