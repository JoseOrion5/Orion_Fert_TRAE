...
def _build_tier3_alchemy_forms(
    ...
    *,
    ...
    forced_quelante_idx: Optional[int] = None
    forced_quelante_lb_kg = 0.0
    if (targets.get("Ca", 0.0) or 0.0) > 0:
        q = None
        for abbr in ("HBED", "EDTA", "HEDTA", "DTPA", "NTA"):
            hit = next((a for a in aditivos if (a.abreviatura or "").strip().upper() == abbr), None)
            if hit:
                q = hit
                break
        if q is None:
        ...
        if q:
            ...
            if dose_pct is None or dose_pct <= 0:
                dose_pct = 0.05
            forced_quelante_lb_kg = ((volume_l * float(dose_pct)) / 100.0) * 1.5
            if forced_quelante_lb_kg > 0:
                nm = f"{q.nome} ({q.abreviatura})" if (q.abreviatura or "").strip() else (q.nome or "Quelante")
    ...
    strategies = [
        {"keys": ["sorbitol", "glicerina"], "relief": 0.015},
        {"keys": ["peg", "polietilenoglicol", "propilenoglicol"], "relief": 0.030},
        {"keys": ["dmso", "nmp"], "relief": 0.050},
        {"keys": ["liquido ionico", "ionic liquid", "cloreto de colina", "choline chloride", "des"], "relief": 0.080},
    ]
    ...
    max_saturation_index = float(max_saturation_index or 0.0)
    max_saturation_index = max(0.05, min(1.2, max_saturation_index))
    solver_sat_limit = 1.2
    target_sat_limit = 0.80
    ...
    for level in range(4):
        ...
        if has_sol_limits:
            ...
            for j in range(num_vars):
                ...
                row.append((1.0 / float(lim)) if (lim is not None and lim > 0) else 0.0)
            A_ub.append(row)
            b_ub.append(float(solver_sat_limit))
        ...
        def solve_with_bounds(local_bounds: List[Tuple[float, Optional[float]]]):
            return linprog(c, A_ub=A_ub, b_ub=b_ub, A_eq=A_eq, b_eq=b_eq, bounds=local_bounds, method="highs")

        res = solve_with_bounds(bounds)
        if not res.success:
            forms.append([])
            continue

        if has_sol_limits and solvent_idx is not None:
            def adjusted_sat(x: Sequence[float]) -> float:
                s = 0.0
                for j in range(num_vars):
                    if j == solvent_idx:
                        continue
                    ...
                    if lim is None or lim <= 0:
                        continue
                    s += float(max(0.0, float(x[j]))) / float(lim)
                s -= float(max(0.0, float(x[solvent_idx]))) * float(sat_relief_per_kg)
                return float(s)

            if adjusted_sat(res.x) > float(target_sat_limit):
                ub_solvent = float(volume_l) * 0.15
                for pct in (0.05, 0.10, 0.15):
                    lb_solvent = min(ub_solvent, float(volume_l) * float(pct))
                    new_bounds = list(bounds)
                    new_bounds[solvent_idx] = (float(lb_solvent), float(ub_solvent))
                    res2 = solve_with_bounds(new_bounds)
                    if res2.success and adjusted_sat(res2.x) <= float(target_sat_limit):
                        res = res2
                        bounds = new_bounds
                        break
